...
/* ── 장르 → 인종 범주 정규화 ── */
const RACE_LIST = ["Fiction", "Non-Fiction", "Poetry", "History", "Philosophy", "Science", "Essay"];
function simRace(id) {
  return RACE_LIST[hash(id) % RACE_LIST.length];
}
function normalizeRace(genre, id) {
  if (genre) {
    const g = genre.toLowerCase();
    if (g.includes("fiction") || g.includes("소설"))          return "Fiction";
    if (g.includes("non-fiction") || g.includes("nonfiction")) return "Non-Fiction";
    if (g.includes("poet") || g.includes("시집"))             return "Poetry";
    if (g.includes("histor") || g.includes("역사"))           return "History";
    if (g.includes("philos") || g.includes("철학"))           return "Philosophy";
    if (g.includes("science") || g.includes("과학"))          return "Science";
    if (g.includes("essay") || g.includes("에세이"))          return "Essay";
  }
  return simRace(id);
}
...
export function toInmate(book) {
  return {
    ...
    inmateNumber: book.callNo || formatInmateNumber(book.id),
    ...
    age:          book.pubDate ? String(2026 - parseInt(book.pubDate, 10)) : "—",
    race:         normalizeRace(book.genre, book.id),
    sex:          hash(book.id) % 2 === 0 ? "Male" : "Female",
    releaseDate:  book.returnDate ? formatReleaseDate(book.returnDate) : "—",
    location:     book.status === "AVAILABLE"   ? "NOT IN NLK CUSTODY"
                : book.status === "CHECKED_OUT" ? "NLK MAIN READING ROOM"
                :                                 "NLK — RESTRICTED ACCESS",
  };
}
